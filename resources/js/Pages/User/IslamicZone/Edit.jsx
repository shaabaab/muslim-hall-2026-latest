...
export default function Edit({ resource, auth, langs }) {
    console.log("language", resource.lang_id, langs);

    const [mainImagePreview, setMainImagePreview] = useState(
        getS3PublicUrl(resource.image)
    );
    const [galleryPreviews, setGalleryPreviews] = useState(
        resource.gallery
            ? resource.gallery.map((img) => getS3PublicUrl(img))
            : [],
    );
    const [documentPreview, setDocumentPreview] = useState(
        getS3PublicUrl(resource.document_file)
    );
    const [pdfPreview, setPdfPreview] = useState(null);

    const [contentType, setContentType] = useState(resource.type || "");
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        type: resource.type || "",
        calendar_type: resource.calendar_type || "",
        image: null,
        gallery: [],
        document_file: null,
        is_featured: resource.is_featured || false,
        status: "draft",
        lang_id: resource.lang_id || "",
        ...
        videos: [],
        pdfs: [],
        existing_audios: resource.islamic_audios || [],
        existing_videos: resource.islamic_videos || [],
        existing_pdfs: resource.islamic_pdfs || [],
        remove_audios: [],
        remove_videos: [],
        remove_pdfs: [],
        audio_temp_paths: [],
    ...
    // Update content type when resource changes
    useEffect(() => {
        setContentType(resource.type || "");
        setCalendarType(resource.calendar_type || "");
    }, [resource]);
    ...
    const handleRemoveMediaFile = (type, index, uidOrId, isExisting) => {
        if (isExisting) {
            setData({
                ...data,
                ["existing_" + type + "s"]: data["existing_" + type + "s"].filter((f) => f.id !== uidOrId),
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (filesToUpload.length > 0) {
                ...
                // setIsUploading(false); // keep it true for final XHR
            } else {
                // Show modal for small files too
            ...
            Object.keys(data).forEach((key) => {
                ...
                if (key === "gallery" && Array.isArray(value)) {
                    ...
                    value.forEach((file, index) => {
                        // Only append if NOT a large file
                        if (!(file instanceof File && file.size >= CHUNK_THRESHOLD)) {
            ...
            // 4. Submit via standard XHR to handle progress/redirects better
            const xhr = new XMLHttpRequest();
            xhr.open("POST", route("user.islamic-zone.update", resource.id), true);
            xhr.setRequestHeader("X-CSRF-TOKEN", csrfToken);
            xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
            xhr.setRequestHeader("Accept", "application/json");

            // track final submission progress
            ...
            xhr.onload = function () {
                if (xhr.status >= 200 && xhr.status < 300) {
                    // message.success("Islamic content updated successfully");
                    setUploadDone(true);
                    // window.location.href = route("user.islamic-zone.index");
                } else if (xhr.status === 409) {
                     // Inertia Version Conflict - force refresh
                     window.location.href = route("user.islamic-zone.index");
                } else {
                    const response = JSON.parse(xhr.responseText);
                    message.error(response.message || "Error updating item");
                }
            };
    ...
    const handleContentTypeChange = (value) => {
        ...
        // Reset calendar type when changing content type
        if (value !== "calendar") {
            setCalendarType("");
    ...
    const contentTypesConfig = {
        islamicContent: {
            ...
            icon: <BookOutlined />,
            description: "Upload Posts thumbns for Islamic Contents",
            allowedFiles: "image/*",
            fileType: "image",
        },
        calendar: {
                    ...
                    <Col span={24}>
                        ...
                        >
                            ...
                            <Text type="secondary" className="block mt-1">
                                Only image files are allowed for Islamic
                                Contents. Max file size: 5MB
                            </Text>
                        ...
                        <Form.Item label="Multiple PDF Files">
                            <MultipleMediaUpload
                                type="pdf"
                                files={[...data.existing_pdfs, ...data.pdfs]}
                                onRemove={(type, index, idOrUid, isEx) => handleRemoveMediaFile(type, index, idOrUid, isEx)}
                                onUpdate={(type, index, file, idOrUid, isEx) => handleUpdateMediaFile(type, index, file, idOrUid, isEx)}
...
>
                        ...
                        <Col span={24}>
                            ...
                            >
                                ...
                                <Text type="secondary" className="block mt-1">
                                    Add multiple calendar images. Max file size:
                                    5MB per image
                                </Text>
                                    ...
                                    resource.gallery.length > 0 && (
                                        <div className="mt-4">
                                            ...
                                            >
                                                ...
                                                existing image(s) - uploading
                                                new images will replace all
                                            </Text>
                                        </div>
                                    )}
                            </Form.Item>
...
>
                                ...
                                <Text type="secondary" className="block mt-1">
                                    Recommended: High-quality images for content
                                    thumbnail. Max: 5MB
                                </Text>
    ...
    const renderMediaFields = () => {
        return (
            <>
                ...
                <Col span={24}>
                    ...
                    >
                        <Upload
                            ...
                            accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.rtf,.csv,.odt,.ods,.odp,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation,text/plain,text/csv"
                            showUploadList={false}
                            maxCount={1}
                        >
                            <Button icon={<FilePdfOutlined />}>Upload Document / PDF</Button>
                        </Upload>

                        {(documentPreview || resource.document_file) && (
                            <div className="mt-3">
                                <Text strong>Current/Selected: </Text>
                                <Text>{data.document_file?.name || resource.document_file || documentPreview}</Text>
                                <Button type="link" danger icon={<DeleteOutlined />} onClick={handleDocumentRemove}>
                                    Remove
                                </Button>
                            </div>
                        )}
                ...
                <Col span={24}>
                    <Divider orientation="left">Media Content (Audios, Videos, PDFs)</Divider>
                    <Row gutter={[24, 24]}>
                        <Col span={24}>
                            <Form.Item label="Audio Files">
                                <MultipleMediaUpload
                                    type="audio"
                                    files={[...data.existing_audios, ...data.audios]}
                        ...
                        <Col span={24}>
                            <Form.Item label="Video Files">
                                <MultipleMediaUpload
                                    type="video"
                                    files={[...data.existing_videos, ...data.videos]}
                ...
                <Col span={24}>
                    ...
                    >
                        <Upload
                            ...
                            multiple
                        >
                            <Button icon={<PlusOutlined />}>Add Related Images</Button>
                        </Upload>
                        {galleryPreviews.length > 0 && (
                            <div className="mt-4">
                                <Text strong className="block mb-2">New Images:</Text>
                                <div className="flex flex-wrap gap-4">
                                    {galleryPreviews.map((preview, index) => (
                                        <div key={index} className="relative">
                                            <img src={preview} style={{ width: "100px", height: "100px", borderRadius: "8px", objectFit: "cover" }} className="border border-dashed border-gray-300" />
                                            <Button type="link" danger size="small" onClick={() => handleGalleryRemove(index)} style={{ position: "absolute", top: -8, right: -8 }}>×</Button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {resource.gallery && resource.gallery.length > 0 && (
                            <div className="mt-4">
                                ...
                                <div className="flex flex-wrap gap-4">
                                    {resource.gallery.map((img, index) => (
                                        <div key={index} className="relative">
                                            <img src={getS3PublicUrl(img)} style={{ width: "100px", height: "100px", borderRadius: "8px", objectFit: "cover" }} className="border border-dashed border-gray-300" />
                                            <Text type="secondary" className="block text-xs text-center mt-1">Existing</Text>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <Col span={12}>
                            <Form.Item label="Status">
                                <Card size="small" className="bg-yellow-50 border-yellow-200">
                                    <Text strong>Draft / Pending Review</Text>
                                    <Text type="secondary" className="block">
                                        User edit will submit as Draft again. Admin will approve/publish it.
                                    </Text>
                                </Card>
                            </Form.Item>
                        </Col>
