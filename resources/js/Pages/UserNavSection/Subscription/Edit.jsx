...
export default function Edit({ auth, subscription, plans = [], users = [] }) {
    ...
    const submit = () => {
        put(route('user.subscriptions.update', subscription.id), {
            onSuccess: () => {
                message.success('Subscription updated successfully');
            },
            onError: () => {
                console.log(errors.error);
                message.error( errors.error || 'Failed to update subscription');
            }
        });
    };
    ...
    if (!subscription) {
        return (
            <FrontAuthenticatedLayout user={auth.user} header="Renew Subscription">
                <Card>
        ...
        >
            <Card>
                <div className="mb-6">
                    ...
                    <Text type="secondary">
                        {subscription.plan.name} -- ({subscription.plan.validity} days - Tk {subscription.plan.price}) <br />
                        {`Renew subscription for ${subscription.user.name} (${subscription.user.email})`}
                    </Text>
                </div>
                ...
                >
                    ...
                    <Col span={24}>
                        ...
                        >
                            ...
                            {plans.length === 0 && (
                                <Text type="danger">No plans available</Text>
                            )}
                        </Form.Item>
