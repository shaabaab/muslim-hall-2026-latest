...
export default function Index({
    ...
    const handleFilter = () => {
        get(route("user.entries.contest.index", contest.id), {
            preserveState: true,
            preserveScroll: true,
            data: data,
        });
    ...
    //     return route("user.contests.participate.index");
    // }
    ...
    const handleDeclareWinner = (entryId) => {
        router.post(
            route("user.winner.declare", entryId),
            {},
            {
                onSuccess: () => {
    ...
    const handleDeclareWinnerManual = (entryId) => {
        router.post(
            route("user.manual.winner", entryId),
            {},
            {
                onSuccess: () => {
                    message.success("Winner declared successfully");
    ...
    const handleDelete = (id) => {
        router.delete(route("user.entries.contest.index", id), {
            onBefore: () => {
                console.log(id);
            },
            onSuccess: () => {
                ...
                <Space size="small">
                    <Tooltip title="View Details">
                        <Button
                            ...
                            onClick={() =>
                                router.visit(
                                    route("user.entries.show", record.id),
                                )
                            }
                        />
                    </Tooltip>
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="mt-2">
                        <Space>
                            ...
                            <Link
                                href={route("user.contests.participate.index")}
                            >
                                <Button
                                    type="default"
                                    icon={<ReloadOutlined />}
