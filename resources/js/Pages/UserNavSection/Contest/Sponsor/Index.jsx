...
export default function Index({ sponsors, filters, auth }) {
    ...
    const handleDelete = (id) => {
        router.delete(route("user.sponsors.destroy", id), {
            onSuccess: () => {
    ...
    const handleEmailVerification = (id, verify = true) => {
        const url = verify
            ? route("user.sponsors.verify-email", id)
            : route("user.sponsors.unverify-email", id);
            ...
            render: (_, record) => (
                <Space size="small">
                    {/* <Tooltip title="Edit Sponsor">
                        <Link href={route('user.sponsors.edit', record.id)}>
                            <Button 
                                type="primary" 
                                icon={<EditOutlined />} 
                                size="small"
        ...
        <Authenticated user={auth.user} header="Sponsors Management">
            <Card>
                ...
                <div className="flex sm:flex-row flex-col gap-3 justify-between items-center mb-6">
                    ...
                    <Link href={route("user.sponsors.create")}>
                        <Button
                            type="primary"
                            icon={<PlusOutlined />}
                            size="large"
