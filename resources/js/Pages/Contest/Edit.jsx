...
export default function Edit({ auth, contest, prizes, categories, sponsors }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        formats: Array.isArray(contest?.formats) ? contest.formats : [],
    ...
    // Derived booleans — re-computed on every render from current data.type
    ...
    // Handle submission type change — SEPARATE handler so it never
    // clears form_url / formats when called from the useEffect sync
    const handleTypeChange = (value) => {
        // Only clear conditional fields when the user actively changes the type
        setData((prev) => ({
            ...prev,
            type: value,
            form_url: value !== "google_form" ? "" : prev.form_url,
            formats: value !== "manual" ? [] : prev.formats,
        }));
    };
    ...
    const submit = () => {
        ...
        const missingBanners = data.sponsor_ids.filter((sponsorId) => {
            const banner = data.sponsor_banners[sponsorId];
            return !banner || (!banner.file && !banner.existing);
        });
        ...
        // Append formats as array
        data.formats.forEach((fmt, index) => {
            formData.append(`formats[${index}]`, fmt);
        });
        ...
        >
            <Card>
                ...
                <Form layout="vertical" onFinish={submit} className="max-w-2xl">
                    ...
                    {/* Google Form URL — only shown when type is google_form */}
                    {isGoogleForm && (
                        <Form.Item
                    ...
                    {/* Formats — only shown when type is manual */}
                    {isManual && (
                        <Form.Item
                            label="Formats"
                        ...
                        >
                            <Select
                                ...
                                value={data.formats || []} // <-- must be an array
                                onChange={(value) =>
                                    handleFieldChange("formats", value)
                                }
                                allowClear
                            >
                    ...
                    <Form.Item>
                        <Space>
                            <div className="flex flex-col md:flex-row mt-20 md:mt-0 items-center gap-4">
                                <Button
                                    type="primary"
                                    htmlType="submit"
                                    loading={processing}
